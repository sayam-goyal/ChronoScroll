...
async function tryInjectAndScrape(tabId, platform){
  ...
  try { await chrome.scripting.executeScript({target:{tabId}, files:['libs/cs_utils.js', platform==='instagram'?'content/instagram.js':'content/youtube_shorts.js']});
        const ans = await new Promise(r => chrome.tabs.sendMessage(tabId, {type:'SCRAPE_EXTRACT'}, r)); if (ans && ans.ok && ans.data) return ans.data; } catch {}
  try {
    const [result] = await chrome.scripting.executeScript({target:{tabId}, func: () => {
      ...
      const re=/#([\p{L}\p{Nd}_]+)/gu; const t=[]; let m; const cap=out.caption||''; while((m=re.exec(cap))) t.push(m[1].toLowerCase()); out.tags=[...new Set(t)]; return out; } });
...
document.getElementById('detect').addEventListener('click', detectFromCurrentTab);
document.getElementById('m-save').addEventListener('click', async ()=>{
  ...
  await pushItem(item);
  mUrl.value=''; mUser.value=''; mCap.value=''; mTags.value=''; alert('Added.');
});
