...
export async function pushItem(item) {
  const db = (await get(DB_KEY)) || []; ensureId(item);
...
export async function updateItem(id, patch) {
  const db = (await get(DB_KEY)) || []; const idx = db.findIndex(x => x.id===id);
  if (idx >= 0) { db[idx] = Object.assign({}, db[idx], patch); await set({[DB_KEY]: db}); return true; } return false;
}
export async function deleteItem(id) {
  const db = (await get(DB_KEY)) || []; const next = db.filter(x => x.id !== id);
  await set({[DB_KEY]: next}); return next.length;
}
export async function getSettings() {
  const defaults = { platform: 'youtube', autoscroll: false, delayMs: 900, minimized: true, categories: {}, appearance: { theme: 'dark', density: 'comfortable' } };
  const s = (await get(SETTINGS_KEY)) || {}; return Object.assign({}, defaults, s);
}
export async function saveSettings(s) {
  const cur = await getSettings(); await set({[SETTINGS_KEY]: Object.assign({}, cur, s)});
}
export async function saveLogin(login) {
  const cur = (await get(LOGIN_KEY)) || {}; await set({[LOGIN_KEY]: Object.assign({}, cur, login)});
}
