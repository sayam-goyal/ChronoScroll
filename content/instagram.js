...
function scrapeFromViewer() {
  ...
  try {
    ...
    const userEl = dialog.querySelector('header a[href^="/"][role="link"], header a[href^="/"][tabindex="0"]') 
                || dialog.querySelector('a[role="link"][href^="/"][tabindex="0"]');
...
function openFirstReelIfGrid(){
  const a = document.querySelector('main a[href^="/reel/"]'); if (a) { a.click(); return true; } return false;
}
...
async function toNext(){
  ...
  dispatchKey(dialog, 'ArrowRight'); await sleep(50); dispatchKey(dialog, 'PageDown'); await sleep(40); wheelScroll(dialog, 1200);
}
...
chrome.runtime.onMessage.addListener((msg, sender, respond)=>{
  if (msg?.type==='CONTENT_START' && msg.payload?.platform==='instagram'){
    RUNNING=true; TICK=Math.max(250, Number(msg.payload.delayMs||900)); lastUrl=null; sameCount=0; busy=false; setTimeout(loop, 400); respond && respond({ok:true}); return true;
  }
  ...
  if (msg?.type==='SCRAPE_ONCE'){
    try {
      if (isOnInstagramAudioPage()) history.back();
      openFirstReelIfGrid();
      const data = scrapeNow(); chrome.runtime.sendMessage({type:'SAVE_SCRAPE_ITEM', payload: data}); respond && respond({ok:true});
...
(async function checkSessionAutostart(){
  try {
    chrome.runtime.sendMessage({type:'GET_SESSION'}, (resp)=>{
      const s = resp?.data; if (s?.running && s.platform==='instagram'){ RUNNING=true; TICK=Math.max(250, Number(s.delayMs||900)); setTimeout(loop, 600); }
    });
  } catch {}
})();
